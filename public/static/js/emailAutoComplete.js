 ...
 */
 function EmailAutoComplete(options) {
	var _this = this;
	this.config = {
		targetCls      :  '.login-reg .left > input:eq(0)',       // 目标input元素
		parentCls      :  'body',       // 当前input元素的父级类
		hiddenCls      :  '.hiddenCls',       // 当前input隐藏域 
		searchForm     :  '.jqtransformdone', //form表单
		hoverBg        :  'hover',          // 鼠标移上去的背景
		inputValColor  :  'black',              // 输入框输入提示颜色
		mailArr        : ["@qq.com","@163.com","@126.com","@yahoo.com","@gmail.com","@yeah.net","@hotmail.com","@sina.cn","@sina.com","@sohu.com"], //邮箱数组
		isSelectHide   : true,                // 点击下拉框 是否隐藏 默认为true
		callback       : function() {                 // 点击某一项回调函数
            $(_this.config.targetCls).change(); // 触发一下文本框的change事件，以让angular的数据绑定能取到数据 By: 猕猴桃
        }
	};
	this.cache = {
		onlyFlag            : true,     // 只渲染一次
		...
		$(_config.targetCls).each(function(index,item){
			...
			$(item).keyup(function(e){
				...
				if(targetVal == '') {
					...
					// 渲染下拉框内容
					self._renderHTML({keycode:keycode,e:e,target:target,targetVal:targetVal,height:elemHeight,width:elemWidth,parentNode:parentNode,top:$(this).offset().top-1,left:$(this).offset().left});
				}
	...
	_renderHTML: function(cfg) {
		...
		if(curIndex > -1){
			...
			if(_cache.onlyFlag) {
				...
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','top':cfg.top + cfg.height + 'px','width':cfg.width + 'px','left':cfg.left,
		...
		if(/@/.test(curVal)) {
			...
			if(newArrs.length > 0) {
				...
				for(var j = 0, jlen = newArrs.length; j < jlen; j++) {
					html += '<li class="p-index'+j+'">'+'<span class="output-num"></span><span class="em" data-html="'+newArrs[j]+'">'+newArrs[j]+'</span></li>';
				}
